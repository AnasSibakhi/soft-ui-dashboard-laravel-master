...
<div class="row">
                ...
                <table class="table table-hover align-items-center mb-0">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder">Question Title</th>
                            <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder">Question Answer</th>
                            <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder">Right Answer</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                @if($question->quiz)
                                <a href="{{ route('quizzes.show', $question->quiz->id) }}" class="course-link">
                                    {{ $question->quiz->name }}
                                </a>
                                @else
                            ...
                            <td class="text-center">
                                <a href=""
                                   class="course-link text-wrap d-inline-block"
    ...
    @elseif($question->type === 'checkbox')
        {{ is_array($answers) ? implode(', ', $answers) : '' }}
    @endif
</td>
                            ...
                            {{-- Right Answer --}}
                            <td class="text-center">
                                <span class="badge bg-success text-uppercase">{{ $question->right_answer }}</span>
                            </td>
                            ...
                            <td class="text-center">
                                ...
                                <a href="{{ route('question.edit', $question->id) }}"
                                   class="btn btn-link text-secondary text-xs px-2" title="Edit">
                                ...
                                <form method="POST" action="{{ route('question.destroy', $question->id) }}" style="display:inline-block;">
                                    @csrf
                                    @method('DELETE')
                                    <button class="btn btn-link text-danger text-xs px-2"
                                            title="Delete"
